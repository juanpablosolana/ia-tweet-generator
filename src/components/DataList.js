export default function DataList ({ handlerMagic }) {
  return (
    <form className='space-y-4 md:space-y-6' onSubmit={handlerMagic}>
      <div>
        <label htmlFor='topics' className='block mb-2 text-sm font-medium text-gray-900 dark:text-white'>Type a subject for which you would like to generate a tweet:</label>
        <input name='topics' id='topics' className='bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500' placeholder='e.g: pets, Beethoven history, mexican food' required='' />
      </div>
